...
def bivariate_analysis(df, app_columns, total_dl_col, total_ul_col):
    ...
    for i, app_column in enumerate(app_columns):
        ...
        axs[i, 0].scatter(df[app_column + ' DL (Bytes)'], df[total_dl_col],
        ...
        axs[i, 0].set_xlim([0, df[app_column + ' DL (Bytes)'].max()])
        axs[i, 0].set_ylim([0, df[total_dl_col].max()])

        # Scatter plot for App UL vs. Total UL
        axs[i, 1].scatter(df[app_column + ' UL (Bytes)'], df[total_ul_col],
        ...
        axs[i, 1].set_xlim([0, df[app_column + ' UL (Bytes)'].max()])
